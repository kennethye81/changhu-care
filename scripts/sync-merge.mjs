...
function mergeMessagesByPatient(local, remote, p7AlertActive) {
...
function mergeResolvedAlertIds(a = [], b = []) {
  return [...new Set([...a, ...b])];
}

function careLogKey(log) {
  return `${log.date}|${log.time}|${log.type}|${log.author}|${log.detail}`;
}

function mergeSubmittedCareLogs(local, remote) {
  const patientIds = new Set([
    ...Object.keys(local || {}).map(Number),
    ...Object.keys(remote || {}).map(Number),
  ]);
...
export function mergeDemoSyncPayload(base, incoming) {
  ...
  return {
    ...
    submittedCareLogs: mergeSubmittedCareLogs(base.submittedCareLogs, incoming.submittedCareLogs),
  };
}
