...
export function getVisiblePatientIds(role: Role, account: string): number[] {
  switch (role) {
    ...
    case 'nurse': return [1, 2];
    case 'assessor': return [1, 2];
    case 'care_worker': return [1, 2];
    case 'family': return [1, 2];
    case 'rehab_therapist': return [1, 2];
    case 'dietitian': return [1, 2];
    case 'case_manager': return [1, 2];
    default: return [1, 2];
  }
}
