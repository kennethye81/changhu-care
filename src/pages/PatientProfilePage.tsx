...
const Overview: FC<{ patient: PatientFull; family: FamilyContact[]; isCrit: boolean; displayPatient: PatientFull }> = ({ patient, family, isCrit, displayPatient }) => {
  ...
  return (<div className="space-y-4"><ST title="Patient Overview" icon={User}/>
    ...
    <div className="grid grid-cols-2 gap-4">
      <div className="glass-card rounded-xl border border-slate-200 p-4"><h3 className="text-xs font-bold text-slate-600 mb-3">Basic Information</h3>
        <div className="space-y-2 text-xs">{[{l:'Name',v:patient.name},{l:'Gender',v:patient.gender},{l:'Age',v:`${patient.age} years`},{l:'Diagnosis',v:patient.diagnosis},{l:'ICD-10',v:patient.diagnosisCodes.join(', ')},{l:'Allergies',v:patient.allergies.join(', ')||'None'},{l:'NEWS Score',v:`${newsScore} (${newsLabel})`},{l:'Monitoring',v:monitoringLabel},{l:'Escalation',v:escalation},{l:'AVPU',v:v.avpu ?? 'A'},{l:'SpO₂ Scale',v:String(v.spo2Scale ?? 2)}].map((r,i)=>(<div key={i} className="flex justify-between gap-3"><span className="text-slate-400 flex-shrink-0">{r.l}</span><span className="font-semibold text-slate-700 text-right">{r.v}</span></div>))}</div>
      </div>
      <div className="glass-card rounded-xl border border-slate-200 p-4"><h3 className="text-xs font-bold text-slate-600 mb-3">Family Contact</h3>
        {family.length === 0 ? (
    ...
    <div className="grid grid-cols-3 gap-4">
      <div className="glass-card rounded-xl border border-slate-200 p-4"><h3 className="text-xs font-bold text-slate-600 mb-2 flex items-center gap-1.5"><BedDouble className="w-3.5 h-3.5 text-indigo-500"/>Sleep</h3><div className="space-y-1.5 text-xs">{sleepData.map((s,i)=>(<div key={i} className="flex justify-between"><span className="text-slate-400">{s.l}</span><span className="font-semibold text-slate-700">{s.v}</span></div>))}</div></div>
