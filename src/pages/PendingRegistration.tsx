                  ...
                  {matchingActive && (
                    <div className="px-5 py-4 bg-warm-50 border-b border-slate-200">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-[10px] font-semibold text-teal-700">AI智能匹配中...</span>
                    ...
                    <div className="w-[55%] overflow-y-auto p-3 border-r border-slate-200 space-y-3 bg-white">
                      {[
                        { role: 'Case Manager', color: 'teal', staff: [
                          { id:'CM001', name:'Grace Tang', age:40, gender:'Female', role:'CM', spec:'照护协调与出院规划', exp:12, avatar:'/avatars/grace-tang.png' },
                          { id:'CM002', name:'Tony Lam', age:43, gender:'Male', role:'CM', spec:'复杂病例与安宁疗护', exp:15, avatar:'/avatars/tony-lam.png' },
                          { id:'CM003', name:'Anna Leung', age:38, gender:'Female', role:'CM', spec:'老年居家护理与慢病管理', exp:10, avatar:'/avatars/anna-leung.png' },
                        ]},
                        { role: 'Registered Nurse (RN)', color: 'amber', staff: [
                          { id:'N006', name:'Jessie Fong', age:34, gender:'Female', role:'RN', spec:'老年与慢病', exp:7, avatar:'/avatars/rn-vacant-1.png' },
                          { id:'N007', name:'Maggie Cheung', age:41, gender:'Female', role:'RN', spec:'术后与安宁', exp:8, avatar:'/avatars/rn-vacant-2.png' },
                          { id:'N008', name:'Brian Ng', age:29, gender:'Male', role:'RN', spec:'心脏与呼吸', exp:6, avatar:'/avatars/rn-vacant-3.png' },
                          { id:'N002', name:'Jenny Tam', age:29, gender:'Female', role:'RN', spec:'居家护理', exp:5, avatar:'/avatars/jenny-tam.png' },
                          { id:'N004', name:'Connie Cheung', age:42, gender:'Female', role:'RN', spec:'伤口护理与用药', exp:14, avatar:'/avatars/connie-cheung.png' },
                        ]},
                        { role: 'Rehab Therapist (PT)', color: 'emerald', staff: [
                          { id:'R006', name:'Catherine Tsang', age:37, gender:'Female', role:'PT', spec:'骨科与神经', exp:7, avatar:'/avatars/pt-vacant-1.png' },
                          { id:'R007', name:'Samson Hui', age:45, gender:'Male', role:'PT', spec:'老年与防跌倒', exp:9, avatar:'/avatars/pt-vacant-2.png' },
                          { id:'R008', name:'Jason Chan', age:32, gender:'Male', role:'PT', spec:'心脏与肺部', exp:5, avatar:'/avatars/pt-vacant-3.png' },
                          { id:'R003', name:'Raymond Wong', age:45, gender:'Male', role:'PT', spec:'脑卒中康复与活动', exp:12, avatar:'/avatars/raymond-wong.png' },
                        ]},
                        { role: 'Care Worker', color: 'purple', staff: [
                          { id:'CW007', name:'Alice Ho', age:43, gender:'Female', role:'CW', spec:'老年居家照护', exp:6, avatar:'/avatars/cw-vacant-1.png' },
                          { id:'CW008', name:'Michelle Yuen', age:38, gender:'Female', role:'CW', spec:'脑卒中后与认知症', exp:8, avatar:'/avatars/cw-vacant-2.png' },
                          { id:'CW009', name:'Kenny Ma', age:48, gender:'Male', role:'CW', spec:'术后与伤口', exp:5, avatar:'/avatars/cw-vacant-3.png' },
                          { id:'CW002', name:'Lisa Cheng', age:38, gender:'Female', role:'CW', spec:'个人照护与陪伴', exp:7, avatar:'/avatars/lisa-cheng.png' },
                          { id:'CW003', name:'Carol Ng', age:45, gender:'Female', role:'CW', spec:'认知症与安宁疗护', exp:10, avatar:'/avatars/carol-ng.png' },
                        ]},
                      ].map(group => (
                        <div key={group.role}>
      ...
      <div ref={setNodeRef} className={`w-[45%] flex flex-col min-h-0 transition-colors ${isOver ? 'bg-[#FFFFFF]' : 'bg-slate-50'}`}>
        ...
        <div className="flex-shrink-0 border-t border-slate-200 px-3 py-2 space-y-2">
          ...
          {matchingActive && matchingProgress < 100 && (
            <div>
              <div className="flex items-center justify-between mb-1">
                <span className="text-[9px] font-semibold text-teal-700">AI匹配中...</span>
              ...
              <div className="space-y-0.5">
                {['诊断与专科','既往病史','服务与角色','经验年限','可用性'].map((d, i) => (
          ...
          {(!matchingActive || matchingProgress >= 100) && Object.keys(matchScores).length > 0 && (
            <div className="pb-1">
              <p className="text-[9px] font-bold text-emerald-700 mb-1">{'\u2713'} AI匹配完成</p>
              {[
          ...
          <div className="flex gap-2">
            <button onClick={() => { setMatchingActive(true); setMatchingProgress(0); setMatchingDim(0); setMatchScores({}); const dims = ['诊断与专科','既往病史','服务与角色','经验年限','可用性']; let step = 0; const t1 = setInterval(() => { step++; setMatchingDim(step); setMatchingProgress(prev => Math.min(prev + 20, 100)); if (step >= dims.length) { clearInterval(t1); const allStaff = [{id:'CM001',name:'Grace Tang',age:40,gender:'Female',role:'CM',spec:'照护协调与出院规划',exp:12,avatar:'/avatars/grace-tang.png'},{id:'CM002',name:'Tony Lam',age:43,gender:'Male',role:'CM',spec:'复杂病例与安宁疗护',exp:15,avatar:'/avatars/tony-lam.png'},{id:'CM003',name:'Anna Leung',age:38,gender:'Female',role:'CM',spec:'老年居家护理与慢病管理',exp:10,avatar:'/avatars/anna-leung.png'},{id:'N006',name:'Jessie Fong',age:34,gender:'Female',role:'RN',spec:'老年与慢病',exp:7,avatar:'/avatars/rn-vacant-1.png'},{id:'N007',name:'Maggie Cheung',age:41,gender:'Female',role:'RN',spec:'术后与安宁',exp:8,avatar:'/avatars/rn-vacant-2.png'},{id:'N008',name:'Brian Ng',age:29,gender:'Male',role:'RN',spec:'心脏与呼吸',exp:6,avatar:'/avatars/rn-vacant-3.png'},{id:'N002',name:'Jenny Tam',age:29,gender:'Female',role:'RN',spec:'居家护理',exp:5,avatar:'/avatars/jenny-tam.png'},{id:'N004',name:'Connie Cheung',age:42,gender:'Female',role:'RN',spec:'伤口护理与用药',exp:14,avatar:'/avatars/connie-cheung.png'},{id:'R006',name:'Catherine Tsang',age:37,gender:'Female',role:'PT',spec:'骨科与神经',exp:7,avatar:'/avatars/pt-vacant-1.png'},{id:'R007',name:'Samson Hui',age:45,gender:'Male',role:'PT',spec:'老年与防跌倒',exp:9,avatar:'/avatars/pt-vacant-2.png'},{id:'R008',name:'Jason Chan',age:32,gender:'Male',role:'PT',spec:'心脏与肺部',exp:5,avatar:'/avatars/pt-vacant-3.png'},{id:'R003',name:'Raymond Wong',age:45,gender:'Male',role:'PT',spec:'脑卒中康复与活动',exp:12,avatar:'/avatars/raymond-wong.png'},{id:'CW007',name:'Alice Ho',age:43,gender:'Female',role:'CW',spec:'老年居家照护',exp:6,avatar:'/avatars/cw-vacant-1.png'},{id:'CW008',name:'Michelle Yuen',age:38,gender:'Female',role:'CW',spec:'脑卒中后与认知症',exp:8,avatar:'/avatars/cw-vacant-2.png'},{id:'CW009',name:'Kenny Ma',age:48,gender:'Male',role:'CW',spec:'术后与伤口',exp:5,avatar:'/avatars/cw-vacant-3.png'},{id:'CW002',name:'Lisa Cheng',age:38,gender:'Female',role:'CW',spec:'个人照护与陪伴',exp:7,avatar:'/avatars/lisa-cheng.png'},{id:'CW003',name:'Carol Ng',age:45,gender:'Female',role:'CW',spec:'认知症与安宁疗护',exp:10,avatar:'/avatars/carol-ng.png'}]; const diag = (p?.diagnosis || '').toLowerCase(); const mhEntries = (p as any)?.medicalHistory || []; const mh = typeof mhEntries === 'string' ? mhEntries.toLowerCase() : mhEntries.map((e: any) => (e.diagnosis || '') + ' ' + (e.chiefComplaint || '')).join(' ').toLowerCase(); const svc = (p?.services || '').toLowerCase(); const diagWords = diag.split(/[\s,·]+/).filter((w:string) => w.length > 2); const mhWords = mh.split(/[\s,·]+/).filter((w:string) => w.length > 2); const svcWords = svc.split(/[\s,·]+/).filter((w:string) => w.length > 2); const scores: Record<string,number> = {}; allStaff.forEach(s => { const team = Object.values(CARE_TEAM).find(t => t.id === s.id); const certs = team?.certifications?.join(' ').toLowerCase() || ''; const bio = team?.bio?.toLowerCase() || ''; const specWords = (s.spec + ' ' + certs + ' ' + bio).toLowerCase().split(/[\s,&]+/); const diagMatch = diagWords.filter((w:string) => specWords.some(sw => sw.includes(w) || w.includes(sw))).length; const diagScore = Math.round((diagMatch / Math.max(diagWords.length, 1)) * 35); const mhMatch = mhWords.filter((w:string) => certs.includes(w)).length; const mhScore = Math.min(Math.round((mhMatch / Math.max(mhWords.length || 1, 1)) * 15), 15); const svcMatch = svcWords.filter((w:string) => specWords.some(sw => sw.includes(w))).length; const svcScore = Math.min(Math.round((svcMatch / Math.max(svcWords.length || 1, 1)) * 15), 15); const expScore = Math.min(s.exp * 2, 20); const availScore = Math.min(Math.floor(Math.random() * 6) + 5, 10); scores[s.id] = Math.min(diagScore + mhScore + svcScore + expScore + availScore, 98); }); setMatchScores(scores); setTimeout(() => { setMatchingActive(false); }, 400); } }, 700); }} className="flex-1 text-[9px] font-semibold bg-gold-500/20 text-gold-700 py-1.5 rounded-lg border border-gold-500/30 hover:bg-gold-500/30 transition-colors">{'\u2728'} AI智能匹配</button>
