...
export function buildFamilyHomeVitalCards(vitals: Vitals, p7Alert: boolean): FamilyVitalCard[] {
  ...
  const card = (
    ...
    unit,
    icon,
    color: isAbnormal ? '#ef4444' : '#C49A6C',
    bg: isAbnormal ? 'bg-red-50' : 'bg-[#FDF5E8]',
    textColor: isAbnormal ? 'text-red-600' : 'text-[#C49A6C]',
    abnormal: isAbnormal,
    trend,
    trendDir,
    ...extra,
...
export function buildFamilyDetailVitalCards(vitals: Vitals, p7Alert: boolean, diagnosis = 'COPD'): FamilyVitalCard[] {
  ...
  const detailMeta = [
    ...
    },
    {
      ...
      unit: 'mg/dL',
      icon: Activity,
      color: glucoseAbnormal ? '#ef4444' : '#9C7A4E',
